...
function SuperAdmin() {
    ...
    const registrarTaller = (e) => {
        ...
        if (!nombre || !emailContacto) {
            toast.warning("El nombre del taller y el correo de contacto son obligatorios.")
            return
        }

        const toastId = toast.loading("Registrando nuevo inquilino y enviando credenciales...")
        ...
        createTaller(nuevoTaller)
            .then(() => {
                toast.success("¡Taller registrado! Se envió la clave temporal por email al dueño.", { id: toastId, duration: 5000 })
                setNombre('')
                setTitular('')
                setTelefono('')
                setEmailContacto('')
            ...
            .catch((err) => {
                const msg = err.response?.data?.error || err.response?.data?.message || "Error al registrar el taller.";
                    ...
                    <div className="sa-card">
                        ...
                        <form onSubmit={registrarTaller} className="sa-form">
                            ...
                            <button type="submit" className="sa-btn-submit">
                                <Plus size={18} /> Crear Taller y Enviar Accesos
                            </button>
                        </form>
                    </div>
