...
function Servicios() {
  ...
  const manejarGuardado = () => {
    ...
    const request = modoEdicion ? api.put(url, servicioAEnviar) : api.post(url, servicioAEnviar);
  ...
  const manejarSubidaMasiva = () => {
    ...
    api.post('/servicios/importar', formData)
    .then(() => {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px', marginBottom: '20px' }}>
        ...
        <div className="tb-card" style={{ background: modoEdicion ? '#fff7ed' : '#ffffff', border: modoEdicion ? '2px solid #fdba74' : '1px solid #e2e8f0', padding: '20px', margin: 0 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 2fr 1fr 1fr 1fr', gap: '15px' }}>
            ...
            <div><label className="tb-label" style={{color: '#166534'}}>Precio A</label><input type="number" value={nuevoServicio.precioA} onChange={e => setNuevoServicio({...nuevoServicio, precioA: e.target.value})} className="tb-input" style={{border: '2px solid #bbf7d0'}} /></div>
            <div><label className="tb-label" style={{color: '#b45309'}}>Precio B</label><input type="number" value={nuevoServicio.precioB} onChange={e => setNuevoServicio({...nuevoServicio, precioB: e.target.value})} className="tb-input" style={{border: '2px solid #fde047'}} /></div>
            <div><label className="tb-label" style={{color: '#6b21a8'}}>Precio C</label><input type="number" value={nuevoServicio.precioC} onChange={e => setNuevoServicio({...nuevoServicio, precioC: e.target.value})} className="tb-input" style={{border: '2px solid #d8b4fe'}} /></div>
            ...
            <div className="tb-table-wrapper" style={{ maxHeight: '500px', overflowY: 'auto' }}>
                ...
                <thead className="tb-thead" style={{ position: 'sticky', top: 0, zIndex: 1 }}>
                    <tr>
                        ...
                        <th className="tb-th" style={{ textAlign: 'right' }}>Precio A</th>
                        <th className="tb-th" style={{ textAlign: 'right' }}>Precio B</th>
                        <th className="tb-th" style={{ textAlign: 'right' }}>Precio C</th>
                ...
                <tbody>
                    ...
                    <tr key={s.id} className="tb-tr">
                        ...
                        <td className="tb-td" style={{ textAlign: 'right', fontWeight: 'bold', color: '#166534' }}>${s.precioA ? s.precioA.toLocaleString() : '0'}</td>
                        <td className="tb-td" style={{ textAlign: 'right', fontWeight: 'bold', color: '#b45309' }}>${s.precioB ? s.precioB.toLocaleString() : '0'}</td>
                        <td className="tb-td" style={{ textAlign: 'right', fontWeight: 'bold', color: '#6b21a8' }}>${s.precioC ? s.precioC.toLocaleString() : '0'}</td>
